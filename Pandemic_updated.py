...
class InfectionEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        return np.concatenate([statuses, masks, vaccines, contacts, [self.economy]], dtype=np.float32)
    ...
    def _calculate_economy(self):
        total = sum(
            p.income_level * (0.2 + 0.8 * (p.health == "susceptible"))
            for p in self.people if p.health != "dead"
        )
        self.economy = (total / self.num_people) * 100 if self.num_people > 0 else 0
    ...
    class Person:
        ...
        def step(self, action=None):
            ...
            mask_level, contact_level, vaccine_level = action

            # Mask level (0=no mask, 1=partial, 2=full)
            mask_compliance_factor = self.compliance_mask / 10
            if mask_level == 0:
                self.mask_usage = max(0.0, self.mask_usage - 0.1 * (1 - mask_compliance_factor))
            elif mask_level == 1:
                self.mask_usage = min(1.0, self.mask_usage + 0.1 * mask_compliance_factor)
            elif mask_level == 2:
            ...
            if contact_level == 0:
                self.social_contacts += int(1 * (1 - lockdown_compliance_factor))
            ...
            # Vaccine level (0=no change, 1=consider partial, 2=consider full)
