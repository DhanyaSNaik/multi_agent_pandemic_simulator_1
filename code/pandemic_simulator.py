...
"""
Please add a professional header here 
"""

class InfectionEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, num_people=100, max_steps=28, decay_rate=0.9999, epsilon=1, gamma=0.9, alpha=0.1):
        super(InfectionEnv, self).__init__()
        self.num_people = num_people
        self.max_steps = max_steps
        # Action space: single action for testing, but each person interprets it as one of 18 individual choices
        self.action_space = spaces.Discrete(18)  
        # Observation space: [status, mask_usage, vaccinated, social_contacts] per person 
        self.observation_space = spaces.Box(
            low=0, high=np.inf, shape=(4 * self.num_people + 1,), dtype=np.float32
        )
        self.people = None
        self.current_step = 0
        self.avg_mask_usage = 0.0 # perhaps in the future we should start this at a random state?
    ...
    def reset(self, seed=None, options=None):
        ...
        # we start with one randomly infected person
    ...
    def step(self, actions=None):
        # takes a list of actions, one per agent
        ...
        # Simulation only terminates when everyone is dead OR after 28 days
    ...
    def _update_health_status(self):
        for p in self.people:
            if p.health == "exposed":
                ...
                # Reinfection model based on time since recovery
                p.days_recovered += 1
    ...
    class Person:
        def __init__(self, id, decay_rate, epsilon, gamma, alpha):
            ...
            # Use defaultdict for faster lookup of missing keys
        ...
        def step(self, action=None):
            ...
            # Handle both tuple actions and integer actions
        ...
        def update_q_table(self, action, reward, next_state):
            ...
            # Use action indices for Q-table keys
            ...
            # Find maximum future Q-value using indices
...
# Function to run training with profiling
def run_training_with_profiling():
    ...
    num_episodes = 100_000  # 100,000 episodes  
    num_people = 100       # 100 agents  
    ...
    save_interval = 10_000   # Save results every 10,000 episodes
    ...
    # Save the initial state AFTER environment is created
    try:
